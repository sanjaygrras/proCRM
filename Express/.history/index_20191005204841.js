...
app.get('/get-subject', (req,res) => {
    const collection = connection.db('procrm').collection('subjects');
    collection.find().toArray( (err,notErr) => {
        if(!err) {
            res.send({status:"ok", msg:"Subject retriving successfully", s:err})
        } else {
            res.send({status:"error", msg:"Something wrong", s:notErr})
        }
    })
})
