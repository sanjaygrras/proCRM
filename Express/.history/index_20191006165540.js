...
app.post('/edit-subject',bodyParser.json(), (req,res) => {
    let collection = connection.db('procrm').collection('subjects');
    console.log(req.body);
    collection.update({_id:ObjectId(req.body._id)},{set:{title:req.body.title, duration: req.body.duration, description: req.body.description, brochure:req.body.brochure}},(err,r)=>{
        if(!err && r)
        {
            res.send({status:"ok", msg:"subject deleted Successfully", data:r});
