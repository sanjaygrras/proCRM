...
app.post('/post-course', upload.single('brochureImage'),(req,res)=>{
    ...
    collection_instance.insertOne(req.body, (err, data)=>{
        if(err){
            ...
            fs.rename(path.join(__dirname,'uploads/temp'+req.file.originalname.substr(req.file.originalname.lastIndexOf('.'))),path.join(__dirname, 'uploads/'+data.insertedId+req.file.originalname.substr(req.file.originalname.lastIndexOf('.'))), (err)=>{
                if(!err)
                {
