...
app.get('/get-subject', (req,res) => {
    ...
    collection.find().toArray( (err,docs) => {
        if(!err) {
            res.send({status:"ok", msg:"Subject retriving successfully", data:docs})
        } else {
            res.send({status:"error", msg:"Something wrong", data:err})
        }
    })
})
...
app.post('/delete-subject', bodyParser.json(), (req,res) => {
    ...
    collection.deleteOne({_id:ObjectId(req.body.dsub)},(err,r)=>{
        console.log(ObjectId(req.body.d));
        if(!err && r)
        {
            res.send({status:"ok", msg:"User deleted Successfully", data:r});
        }
...
app.post('/delete-subject',bodyParser.json(), (req,res) => {
    ...
    collection.deleteOne({_id:ObjectId(req.body.dsub)},(err,r)=>{
        if(!err && r)
        {
            res.send({status:"ok", msg:"User deleted Successfully", d:r});
        }
        else{
            res.send({status:"failed", msg:"some error occured", d:err});
