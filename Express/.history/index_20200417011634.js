...
app.post('/student-register', upload.single('sPhoto'), (req,res) => {
    ...
    if(req.file) {
        ...
        collection.insertOne(sName:req.body.sName, sMobile:req.body.sMobile, sEmail:req.body.sEmail, sRequest:req.body.sRequest,sCourse:req.body.sCourse,sAddress:req.body.sAddress, (err, data) => {
            if(err){
                res.send({status:"failed", message : "Student Can't register"});
            }
            else{
                console.log("Student created succeffully");
