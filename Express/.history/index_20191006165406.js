...
app.post('/delete-subject',bodyParser.json(), (req,res) => {
    console.log(req.body);
    let collection = connection.db('procrm').collection('subjects');
    console.log({_id:ObjectId(req.body.dsub)});
    collection.deleteOne({_id:ObjectId(req.body.dsub)},(err,r)=>{
        if(!err && r)
        {
            res.send({status:"ok", msg:"subject deleted Successfully", data:r});
            console.log ("deleting successs ssssssssssss");
        }
        else{
...
app.post('/edit-subject',bodyParser.json(), (req,res) => {
    let collection = connection.db('procrm').collection('subjects');
    console.log(req.body);
    collection.update({_id:ObjectId(req.body.dsub)},{set:{title:req.body.title, duration: req.body.duration, description: req.body.description, brochure:req.body.brochure}},(err,r)=>{
        if(!err && r)
        {
            res.send({status:"ok", msg:"subject deleted Successfully", data:r});
            console.log ("updating successs");
        }
        else{
