...
var storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        console.log(fil.originalname.substr(file.originalname.lastIndexOf('.')));
      cb(null, 'temp.jpg')
    }
  })
...
app.post('/post-course', upload.single('brochureImage'),(req,res)=>{
    ...
    collection_instance.insertOne(req.body, (err, data)=>{
        if(err){
            ...
            console.log(data);
            fs.rename(path.join(__dirname,'uploads/temp.jpg'),path.join(__dirname, 'uploads/'+data.insertedId+".jpg"), (err)=>{
                if(!err)
                {
